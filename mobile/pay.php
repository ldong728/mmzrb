    ...
    if ('SUCCESS' == $responseData['return_code']) {
        if ('SUCCESS' == $responseData['result_code']) {
            ...
            if (signVerify($responseData)) {
                ...
                $payChkArray=array(
                    'first'=>array('value'=>'您在anmiee海外购商城的网购订单已支付成功：'),
...
//                mylog(json_encode($payChkArray));
                $re=sendTemplateMsg($responseData['openid'],'gPTK3iniMGqqNGl8V4cWU0Xt4d4OTJLeTjdEiFwXOcE','',$payChkArray);
                mylog($re);
